...
func UnpackReflect(u interface{}, data []byte) error {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if typeField.Tag.Get("unpack") == "-" {
			// Если бы в data было бы значение для этого поля, то нужно было бы вычитать это значение,
			// используя binary.Read(r, binary.LittleEndian, &value) иначе, значение этого поля осталось бы
			// в reader и тогда бы оно было бы прочитано при следующей итерации
			continue
		}
